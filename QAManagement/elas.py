...
##################插入函数##############
    def insert(self,myindex,myquestion,myaccuratequestion,myanswer,mylink,mysubject,myid):###变量为我们的插入.参数为q,a,id,注意此处的id
        doc = {
            "question": myquestion ,
    ...
    def furthersearch(self,myindex,s1):
        res = self.es.search(index=myindex, from_=1,size=5, body={"query": {"match": {"question": s1}}})
    ...
    def updateqa(self, myindex, myquestion,myanswer, mylink,mysubject,hitid):  # 修改问题，同时精确问题域也要被修改
        res = self.es.update(index=myindex, doc_type='user', id=hitid,
                            body={"doc": {"question": myquestion, "accuratequestion": myquestion,"answer":myanswer,"link":mylink,"subject":mysubject}})
